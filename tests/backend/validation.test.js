...
// Works for both Zod "issues" (newer) and "errors" (older)
...
describe('Signup Schema Validation', () => {
  ...
  it('rejects invalid email format', () => {
    ...
    expect(firstErrorMessage(result)).toBe('Invalid email format');
  });
  ...
  it('rejects username shorter than 3 characters', () => {
    ...
    const result = signupSchema.safeParse(invalidData);
    expect(result.success).toBe(false);
    expect(firstErrorMessage(result)).toBe('Username must be at least 3 characters');
  });
  ...
  it('rejects password shorter than 6 characters', () => {
    ...
    const result = signupSchema.safeParse(invalidData);
    expect(result.success).toBe(false);
    expect(firstErrorMessage(result)).toBe('Password must be at least 6 characters');
  });
});
...
describe('Login Schema Validation', () => {
  ...
  it('rejects empty identifier', () => {
    ...
    expect(result.success).toBe(false);
    expect(firstErrorMessage(result)).toBe('Email or username is required');
  });
  ...
  it('rejects empty password', () => {
    ...
    const result = loginSchema.safeParse(invalidData);
    expect(result.success).toBe(false);
    expect(firstErrorMessage(result)).toBe('Password is required');
  });
});
...
describe('Create Recipe Schema Validation', () => {
  ...
  it('rejects recipe without ingredients', () => {
    ...
    const result = createRecipeSchema.safeParse(invalidRecipe);
    expect(result.success).toBe(false);
    expect(firstErrorMessage(result)).toBe('At least one ingredient is required');
  });

  it('rejects recipe without steps', () => {
    const invalidRecipe = { ...validRecipe, steps: [] };
    const result = createRecipeSchema.safeParse(invalidRecipe);
    expect(result.success).toBe(false);
    expect(firstErrorMessage(result)).toBe('At least one step is required');
  });

  it('accepts recipe with optional tags array', () => {
    const recipeWithTags = { ...validRecipe, tags: ['dessert', 'easy', 'quick'] };
